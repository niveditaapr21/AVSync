...
from scripts.train_utils import (EarlyStopper, AverageMeter,
                                 ...
                                 set_seed, toggle_mode, verbose_epoch_progress_r,  verbose_epoch_progress_c, apply_fn_recursive,
...
def train(cfg):
    ...
    elif cfg.action=='train_avsync_model_r':
        #target_key = 'offset_target'
        target_key = 'offset_sec'
    else:
        target_key = 'offset_target'
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in phases_to_run_on:
                ...
                for i, batch in enumerate(loaders[phase]):
                    ...
                    with torch.autocast('cuda', enabled=cfg.training.use_half_precision):
                        with torch.set_grad_enabled(phase == 'train'):
                            #print(f"dekho2 {targets[target_key]}")
            ...
            if cfg.action == 'train_avsync_model_r':
                epoch_loss, metrics = verbose_epoch_progress_r(cfg, global_rank, logger, running_results, phase, epoch,
                                                         datasets['train'], save_csv_with_preds)
            else:
                epoch_loss, metrics = verbose_epoch_progress_c(cfg, global_rank, logger, running_results, phase, epoch,
                                                         datasets['train'], save_csv_with_preds)
