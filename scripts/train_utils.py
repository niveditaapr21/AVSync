...
def verbose_epoch_progress_r(cfg, global_rank, logger, running_results: dict, phase, epoch, train_dataset,
    ...
    # logging metrics
    print(f"dekho4.8: {running_results['targets']}")
    logits = torch.cat(running_results['logits']).float()
    print(f"dekho4.9: {running_results['targets']}")
    targets = torch.cat(running_results['targets'])
    print(f"dekho5: {running_results['targets']}")

    metrics = calc_cls_metrics_r(cfg, targets, logits, only_accuracy=True)
    ...
    if is_task_binary:
        ...
        metrics = calc_cls_metrics_r(cfg, targets, logits)
        ...
        #label2metrics = calc_performance_per_class(cfg, target2label, 'off', logits, targets,
...
def verbose_epoch_progress_c(cfg, global_rank, logger, running_results: dict, phase, epoch, train_dataset,
                           save_csv_with_preds=False):
    running_results = gather_dict(running_results)
    # logging loss values
    if is_master(global_rank):
    ...
    metrics = calc_cls_metrics(cfg, targets, logits, only_accuracy=True)
    ...
    if is_task_binary:
        ...
        off_transf = get_transform_instance_from_compose(train_dataset.transforms, 'TemporalCropAndOffset')
        metrics = calc_cls_metrics(cfg, targets, logits)
        # make a map from targets (int) to labels (human-readable class names)
        label_grid = [str(round(c, 3)) for c in off_transf.class_grid.tolist()]
        target2label = {target: label for target, label in enumerate(label_grid)}
        label2metrics = calc_performance_per_class(cfg, target2label, 'off', logits, targets,
...
def calc_performance_per_class(cfg, target2label, class_category, logits_off, targets_off, targets_cls=None,
    ...
    for key, preds in target_cls2preds.items():
        if len(preds['logits_off']) > 0 and len(preds['targets_off']) > 0:
            ...
            if cfg.action == "train_avsync_model_r":
                metric_name2val = calc_cls_metrics_r(cfg, torch.tensor(preds['targets_off']),
                                               torch.stack(preds['logits_off']),
                                               only_accuracy=True, verbose=False, add_doubt_cls=add_doubt_cls)
            else:
                metric_name2val = calc_cls_metrics(cfg, torch.tensor(preds['targets_off']),
                                               torch.stack(preds['logits_off']),
                                               only_accuracy=True, verbose=False, add_doubt_cls=add_doubt_cls)
        else:
...
def calc_cls_metrics_r(cfg, targets, outputs: torch.FloatTensor, topk=(1, 5), only_accuracy=False, prefix='',
    ...
    tolerance = 0.02
    # Ensure predictions and targets are 1D tensors
    ...
    for i in range(len(predictions)):
        print(f"here prediction, targets ={predictions[i]}-{targets[i]} = {predictions[i]-targets[i]}")
    # Count predictions within tolerance
    within_tolerance = (differences <= tolerance).sum().item()
    print(f"within_tolerance= {within_tolerance}")
    # Calculate accuracy
    total = targets.size(0)
    metrics_dict[f'{prefix}accuracy_1'] = (within_tolerance / total) * 100
    print(f"accuracy = {(within_tolerance / total) * 100}")
    # with open("/home/gnivedita/Synchformer/model/alpha.csv","a") as f:
    #     for i in range(len(predictions)):
    #         f.write(f"{predictions[i]},{targets[i]},{predictions[i]-targets[i]}\n")
    #     f.write(f"within_tolerance= {within_tolerance}\n") 
    #     f.write(f"accuracy = {(within_tolerance / total) * 100}\n") 

    #metrics_dict[f'{prefix}accuracy_1_tol1'] = 0
    return metrics_dict

def calc_cls_metrics(cfg, targets, outputs: torch.FloatTensor, topk=(1, 5), only_accuracy=False, prefix='',
                     verbose=True, add_doubt_cls: bool = False, calc_tol_accuracy=True,
                     softmaxed_outputs=False, calc_pr_rec_f1=False):
    """
    Adapted from https://github.com/hche11/VGGSound/blob/master/utils.py
