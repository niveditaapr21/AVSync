...
class TemporalCropAndOffset(torch.nn.Module):
    ...
    def __init__(self, crop_len_sec: float, max_off_sec: float, offset_type='grid', do_offset: bool = True,
        ...
        #do_offset = False
        #print(f"here1: {self.do_offset} {do_offset}")
        if do_offset:
            #print("here2")
            if offset_type == 'grid':
                ...
                if self.max_a_jitter_sec is not None:
                    #print(f"nivi {self.max_a_jitter_sec} {(self.class_grid[1] - self.class_grid[0]) / 2}")
    ...
    def forward(self, item):
        ...
        if self.do_offset:
            ...
            #offset_sec, v_start_i_sec = None, None
            #print(f'nivi1: {offset_sec} {v_start_i_sec}')
            ...
            if offset_sec is None and v_start_i_sec is None:
                ...
                if self.offset_type == 'grid':
                    ...
                    #print("yes")
                    offset_sec = float(item["path"].split('_')[-2])
                    #print(float(item["path"].split('_')[-2]))
                    #during test
                    #offset_sec = 0
                    #print("yes")
                ...
                #print(f"frame {v_len_frames}, {v_crop_len_frames}, {v_fps}")
                ...
                #print(item['path'])
                #print(f"nivi2: {v_start_max_sec} {offset_sec}")
                ...
                #print("here4")
                #use the following to use offset_sec from file name itself while training
                #ex. video_name: set2_c1_sample1_-0.1_4s.mp4, offset sec: -0.1
                offset_sec = float(item["path"].split('_')[-2])
                #offset_sec = round(offset_sec, 2)
                #print(f"here4:{offset_sec}")
                v_start_sec = 0.4
                v_start_i_sec = 0.4
            ...
            #print(offset_sec)
            #print(f"nivi: {frames2sec(a_start_i, a_fps)}")
            #print(f"nivi: {frames2sec(v_start_i, v_fps)}")
        
        else:
            print("here5")
            #print(item["path"], v_len_frames, v_crop_len_frames)
            if item['split'] == 'test':
                offset_sec = 0.0    #during test
            else:
                offset_sec = float(item["path"].split('_')[-2])    #during training        
            #offset_sec = round(offset_sec, 2)
            #print(f"here:{offset_sec}")
            ...
            #print({item['path']}, offset_sec, v_start_i_sec, offset_label, offset_target)
        ...
        #print(f"nivi: {frames2sec(a_start_i, a_fps)}, {frames2sec(a_crop_len_frames, a_fps)=}, {frames2sec(a_end_i, a_fps)=}")
        #print(f"nivi: {frames2sec(v_start_i, v_fps)}, {frames2sec(v_crop_len_frames, v_fps)=}, {frames2sec(v_end_i, v_fps)=}")
        #frames2sec(v_start_i, v_fps)
        ...
        if self.do_offset:
            ...
            # if offset_sec>0.04:
            #     offset_sec = 0.2
            # elif offset_sec>=-0.125 and offset_sec<=0.04:
            #     offset_sec = 0
            # else:
            #     offset_sec = -0.2
            item['targets']['offset_sec'] = offset_sec
...
class GenerateMultipleSegments(torch.nn.Module):
    ...
    def forward(self, item):
        ...
        # print(f"{self.step_size_seg=}")
        # print(f"{segment_size_vframes=},{segment_size_aframes=},{stride_vframes=},{stride_aframes=},{n_segments=}")
        # print(f"{v_len_frames=},{a_len_frames=}")
        #print(f"{frames2sec(v_ranges[0][0], v_fps)=}, {frames2sec(a_ranges[0][0], a_fps)=}")
        # print(f"{v_ranges=}")
        # print(f"{a_ranges=}")
    ...
    def get_sequential_seg_ranges(self, v_len_frames, a_len_frames, v_fps, a_fps, n_seg, seg_size_aframes):
        ...
        #print(f"{frames2sec(v_start_i, v_fps)=}, {frames2sec(a_start_i, a_fps)=}")
        ...
        if self.audio_jitter_sec > 0:
            ...
            # print(f"{jitter_aframes=},{a_start_i=},{(a_len_frames-a_start_i-aframes_seg_seq_len)=}")
            # print(f"{a_len_frames=},{a_start_i=},{aframes_seg_seq_len=}")
            ...
            #print(f"{a_start_seg_i=}")
            #print(f"{self.audio_jitter_sec=},{jitter_aframes=},{a_start_i=},{(a_len_frames-a_start_i-aframes_seg_seq_len)=}")
